	...
	public Pions meilleurCoup(TableDeJeu jeu, Pions p) {
		...
		if(niv==0) {
			...
			if(niv==1) {
				...
				for(i=0; i<4; i++) {
					for(j=0; j<4; j++) {
						if(jeu.testCaseVide(i, j)) {
							jeu.poserPions(p,i,j);
							if(jeu.testVictoire()) { //Si la case est une case elle la pose 
								posX=i;
								posY=j;
								i=4;
								j=4;
				...
				//Si elle ne trouve pas de de case gagnante elle pose la piece dans une case au hasard
				...
				//elle renvoie un pion qui n est pas un pion gangant 
					...
					if(!jeu.testCaseVide(i)) { 
						...
						for(j=0; j<4; j++) {
							for(k=0; k<4; k++) {
								if(jeu.testCaseVide(j,k)) {
									jeu.poserPions(p, j, k);
									//Si le pion est un pion gangant elle ne le renvoie pas
									if(jeu.testVictoire()) poser = true;
									jeu.annulerPions(p,j,k);
								}
							}
				...
				//elle renvoie un pion au hasard sinon
				i=0;
				do {
					i++;
				}while(jeu.testCaseVide(i));
